...
class BookingTransactionResource extends Resource
{
    ...
    public static function updateTotals(Get $get, Set $set): void
    {
        ...
        $prices = Product::find($selectedproductServices->pluck('product_id'))
            ->pluck('price', 'id');

        $subTotal = $selectedproductServices->reduce(function ($subtotal, $item) use ($prices) {
            return $subtotal + ($prices[$item['product_id']] ?? 0);
        }, 0);
